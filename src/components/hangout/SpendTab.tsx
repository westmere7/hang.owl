        ...
        >
          <div className="space-y-1.5">
            {members.map((m) => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-2.5 rounded-2xl border border-line/50 bg-surface-2/80 px-3 py-2">
                  <Avatar name={m.display_name} size="sm" />
                  <span
                    className={cn(
                      'min-w-0 flex-1 truncate text-sm font-extrabold',
                      w > 0 ? 'text-ink' : 'text-muted line-through opacity-60',
                    )}
                  >
                    {m.display_name}
                  </span>
                  <div className="flex gap-1">
                    {WEIGHT_OPTIONS.map((opt) => (
                      <button
                        ...
                        className={cn(
                          'rounded-xl px-2.5 py-1 text-xs font-black transition-all select-none',
                          w === opt.value
                            ? opt.value === 0
