...
function FullBillRecapModal({
  ...
  async function handleSaveImage() {
    ...
    try {
      ...
      // Calculate natural content height dynamically
      let calculatedHeight = padding + 110 + 140 + 50 // Header & Hero Box
      if (spends.length > 0) {
        calculatedHeight += 32 + spends.length * 66 + 24 // Spends
      }
      calculatedHeight += 32 + recap.rows.length * 66 + 24 // Summary
      calculatedHeight += 32 + Math.max(1, recap.settlements.length) * 66 + 40 // Transfers
      calculatedHeight += 60 + padding // Footer & bottom padding

      const targetHeight = Math.ceil(calculatedHeight)
      canvas.width = width
      ...
      ctx.fillText('Generated by HangOwl App 🦉 · hangowl.app', width / 2, y + 20)
      ctx.textAlign = 'left'
    ...
    <Modal
      ...
      footer={
        <div className="flex flex-col sm:flex-row gap-2.5">
          ...
          >
            <Download size={18} />
            {generatingImg ? 'Exporting image…' : 'Export image'}
          </Button>
          <Button
            type="button"
            variant="outline"
